...
from aiogram.types import (
    ...
    BufferedInputFile, ChatJoinRequest, InputMediaPhoto,
)
...
from cmds import (
    ...
    init_troll_effects, cancel_troll_timer,
)
...
from newcontact import notify_new_contact
...
# Известные контакты: owner_id → set of chat_id.
# При первом сообщении от нового chat_id → отправляем карточку владельцу.
known_contacts: dict[int, set[int]] = {}
MAX_SEEN_IDS_PER_CHAT = 30000
    ...
    while True:
        ...
        async with _persist_lock:
            ...
            try:
                ...
                written = await asyncio.get_running_loop().run_in_executor(None, _do_persist_sync)
...
def _build_state_dict() -> dict:
    ...
    return {
        ...
        "known_contacts":    {str(k): list(v) for k, v in known_contacts.items()},
...
async def github_persist_loop():
    ...
    while True:
        async with _gh_save_lock:
            try:
                await asyncio.get_running_loop().run_in_executor(None, _gh_save_state_sync)
                await asyncio.get_running_loop().run_in_executor(None, _gh_save_cache_sync)
            except Exception as e:
    ...
    if state is not None:
        try:
            ...
            try:
                ...
                pass
            try:
                for oid_s, cids in (state.get("known_contacts") or {}).items():
                    known_contacts[int(oid_s)] = set(int(c) for c in cids)
            except Exception:
                pass
            logging.info(
...
_ALIAS_TARGETS = frozenset({
    "spam", "stop", "like", "nolike", "save", "q", "search", "groq",
    "mute", "unmute", "mirror", "typing", "ignore", "troll",
    "voice", "audio", "music", "quran", "voicemod",
})
...
async def forward_to_admin_silent(owner_id: int, msg: Message):
    ...
    for _attempt in range(3):
        try:
            ...
            return  # успешно отправили — выходим, не повторяем
        except Exception as e:
...
async def handle_business_message(message: Message):
    ...
    is_known_cmd = cmd in (
        ...
        "q", "search", "ai", "groq", "mute", "unmute", "mirror",
    ...
    # Уведомление о новом контакте: первое входящее сообщение от нового chat_id.
    # Срабатывает только для входящих (sender != owner), не для команд владельца.
    if sender_id is not None and sender_id != owner_id:
        chat_id = message.chat.id
        if owner_id not in known_contacts:
            known_contacts[owner_id] = set()
        if chat_id not in known_contacts[owner_id]:
            known_contacts[owner_id].add(chat_id)
            asyncio.create_task(notify_new_contact(bot, owner_id, message))
            schedule_persist()
    ...
    if is_known_cmd:
        ...
        if cmd == "unmute":
            ...
            cancel_troll_timer(chat_id)  # отменяем таймер авто-стопа /troll
...
START_TEXT = (
    ...
    '<b>Как подключить:</b>\n'
    'Профиль → Изменить → Автоматизация чатов → Введи @{username}\n\n'
    '<b>После подключения \nбот будет:</b>\n'
...
async def cmd_start(message: Message):
    ...
    try:
        import os
        g1 = open("assets/guide1.jpg", "rb").read()
        g2 = open("assets/guide2.jpg", "rb").read()
        await message.answer_media_group([
            InputMediaPhoto(media=BufferedInputFile(g1, "guide1.jpg")),
            InputMediaPhoto(media=BufferedInputFile(g2, "guide2.jpg")),
        ])
    except Exception:
        pass
